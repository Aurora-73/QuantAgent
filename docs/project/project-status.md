...
> 生成日期：2026-07-06 | 最后更新：2026-07-07（文档系统性整理）| 范围：quant-system 全模块
> 状态：✅ 当前 | 数据统计以 `python -m scripts.db_stats` 实时输出为准
...
| 项目 | 状态 | 详情 |
|------|------|------|
| **行情数据** | ✅ 正常 | baostock(优先) → AKShare(回退)，行数以 db_stats 为准 |
| **指数数据** | ✅ 正常 | AKShare 回退 |
| **基本面** | ✅ 已接入 | baostock 5 种报表 API，写入 research.financials 表 |
| **估值** | ✅ 正常 | AKShare stock_value_em，PE/PB/市值 |
| **行业分类** | ✅ 正常 | baostock query_stock_industry，全市场映射 |
| **北向资金** | ✅ 正常 | AKShare 北向净流入 |
| **融资融券** | ✅ 正常 | AKShare |
| **新闻** | ✅ 已接入 | news/aggregator.py + AKShareCollector，写入 events 表 |
...
| 项目 | 状态 | 详情 |
|------|------|------|
| **注册因子数** | ✅ 29 个 | 25 OHLCV + 4 基本面 (roe/pe_ttm/revenue_growth/profit_growth)；factors 表实际存储因子数以 db_stats 为准 |
...
| **因子存储** | ✅ 正常 | factors 表含 factor_evaluation 评估结果，行数以 db_stats 为准 |
| **回测记录** | ✅ 正常 | backtest_runs 表，4 策略 × 多标的 |
| **决策记忆** | ✅ 正常 | decision_memory 表，含 1/3/5/10 日回填 |
| **日报** | ✅ 正常 | knowledge/daily/ 目录 |
...
| **EventExtractor** | 📦 已移除 | 原 LLM 新闻事件抽取 — 已随 LLM 模块清理移除（详见 §0.2） |
| **ReportAgent** | 📦 已移除 | 原 LLM 日报生成 (OKF 格式) — 已随 LLM 模块清理移除（详见 §0.2） |
...
| **测试** | ✅ 全部通过 | 11 个测试文件，合成数据，覆盖核心模块（以 `pytest tests/` 输出为准） |
...
> 数据统计为快照性质，**以 `python -m scripts.db_stats` 实时输出为准**。下表仅列出表名与说明，不固化行数（行数会随数据更新变化）。
...
| backtest_runs | 回测结果记录 |
| backtest_equity | 回测权益曲线 |
| events | 事件表（新闻事件冷启动入库） |
| predictions | 预测记录 |
| decision_memory | 决策记忆 |
| market_facts | 市场事实 |
| lessons | 教训库 |
| schema_version | DB 迁移版本 |

> 如需查看当前各表行数，运行：`python -m scripts.db_stats`
...
2. ~~**事件表为空**~~ — ✅ 已解决（2026-07-06），冷启动新闻事件入库（行数以 db_stats 为准），见 `scripts/cold_start_events.py`
3. **预测/决策闭环未启动** — predictions/decision_memory 表行数偏少（以 db_stats 为准），验证闭环不通（需 Linux 服务器跑 daily_research 验证）
...
6. ~~**股票池接近完整**~~ — ✅ 已解决（2026-07-06），股票池与日线记录已入库（行数以 db_stats 为准）
7. ~~**基本面数据不完整**~~ — ✅ 已解决（2026-07-06），基本面报表已接入（行数以 db_stats 为准，Q4+Q1 完整）
