"""
MCP Server — 30 tools for Claude Code integration.
...
mcp.tool(
    name="get_factors",
    description="获取已注册的因子列表或指定因子的数值",
    annotations={"readOnlyHint": True},
)(tools_data.get_factors)

mcp.tool(
    name="get_index_data",
    description="获取指数行情数据（默认沪深300）",
    annotations={"readOnlyHint": True},
)(tools_data.get_index_data)

mcp.tool(
    name="get_universe",
    description="获取系统跟踪的股票列表",
    annotations={"readOnlyHint": True},
)(tools_data.get_universe)

mcp.tool(
    name="get_market_overview",
    description="获取市场概况（指数行情 + 涨跌统计）",
    annotations={"readOnlyHint": True},
...
mcp.tool(
    ...
    description="获取当前市场状态识别结果",
    annotations={"readOnlyHint": True},
)(tools_risk.get_market_regime)

# ============================================================
# Knowledge & Decision Tools (11, read-only)
# ============================================================

mcp.tool(
    name="get_daily_report",
    description="获取指定日期的研究报告",
    annotations={"readOnlyHint": True},
)(tools_knowledge.get_daily_report)

mcp.tool(
    name="search_events",
    description="搜索结构化金融事件",
    annotations={"readOnlyHint": True},
)(tools_knowledge.search_events)

mcp.tool(
    name="wiki_search",
    description="搜索交易方法论文档",
    annotations={"readOnlyHint": True},
